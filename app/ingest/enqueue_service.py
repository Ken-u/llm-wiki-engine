...
async def _execute_ingest_enqueue_with_db(
    ...
    *,
    ...
    from app.ingest.router import EnqueueFilesRequest, _build_file_items, _selection_from_request
    ...
    if body.selection is not None:
        ...
        try:
            resolved = resolve_browser_source_files(enqueue_source_dir, body.source_files)
        except FileNotFoundError as exc:
            raise HTTPException(status.HTTP_404_NOT_FOUND, f"Source file not found: {exc.args[0]}") from None
        except ValueError as exc:
            raise HTTPException(status.HTTP_400_BAD_REQUEST, str(exc)) from None
        items = await _build_file_items(
            project,
            db,
            source_dir=enqueue_source_dir,
            sources=resolved,
        )
    ...
    staging_pairs = [(item.source_path, str(enqueue_source_dir)) for item in eligible]
