"""Tests for case markdown parser numbered headings."""
...
## 2. 适用范围
Android 13, reference device platform.
...
def test_parse_case_numbered_sections(tmp_path):
    record, chunks = parse_case_markdown(
        CASE_MARKDOWN, "raw/sources/2001.md"
    )
    assert record.case_id == "2001"
    assert record.problem_summary == "设备无法正常启动，卡在开机动画。"
    assert record.scope == "Android 13, reference device platform."
